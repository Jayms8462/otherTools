...
for url in data:
    if test == "127.0.0.1":
        subprocess.run(["python", "C:\\Users\\ajpor\\OneDrive\\desktop\\git\\otherTools\\cardData\\grabInfo.py", "-u", os.getenv('user'), "-p", os.getenv('password'), "-U", url, "-D", "cards", "-H", "127.0.0.1", "-P", "27017"])
    elif test != "127.0.0.1":
        # Creds
        host = test
        ...
        stdin, stdout, stderr = con.exec_command('python C:\\Users\\ajpor\\Desktop\git\\otherTools\\cardData\\grabInfo.py -u ' + os.getenv('user') + ' -p ' + os.getenv('password') + ' -U url -D "cards" -H "192.168.1.172" -P "27017"')   
